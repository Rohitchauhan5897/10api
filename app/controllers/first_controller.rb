					  							 ...
					  							 if params[:email].present?	
					 													...
					 													if user.present?
					 											 				...
					 											 				social_user_info=create_social_auth(user,params[:device_type],params[:device_id],authinfo)
	 																				...
	 																				social_info = create_user_with_signup(params[:device_type],params[:device_id],params[:firstname],params[:lastname],params[:username],params[:email],params[:contact_no],params[:gender],params[:dob])
	 																				# send_json_method(code:200,Message:"login Successful")
	 																				render json:{code:200,message:"login Successful",user:social_info}
																			end
					  								end
	end
end
